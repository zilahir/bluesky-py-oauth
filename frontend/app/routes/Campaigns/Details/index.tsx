...
export function meta({ data: metaRouteData }: Route.MetaArgs) {
  const campaign = metaRouteData?.campaign;

  if (campaign) {
    const campaignNameCapitalized =
      campaign.name.charAt(0).toUpperCase() + campaign.name.slice(1);
    return [
      {
        title: `Campaign - ${campaignNameCapitalized}`,
        description: `Details of the campaign ${campaignNameCapitalized}`,
      },
    ];
  }
